...
| [Tauri](https://github.com/tauri-apps/tauri) / [Vite integration](https://v2.tauri.app/start/frontend/vite/) | Native desktop delivery for the shared React dashboard | Keep the webview as a thin UI shell; leave API, collaboration, device, and agent authority in the existing local/client processes | Reuse the maintained Tauri 2 CLI and Rust runtime; no Tauri source is copied | MIT or Apache-2.0 where applicable; compatible with CoCodex MIT, with Cargo/npm lockfiles and notices reviewed before release | The shell grants only `core:default`, embeds the same `gui/dist` output, and uses a loopback API base. It does not receive shell/filesystem permissions or replace the CoCodex Server. Test CSP, `tauri.localhost` CORS, offline proxy behavior, window lifecycle, installer prerequisites, and API-origin isolation. |
