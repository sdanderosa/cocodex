...
| [libsodium AEAD](https://doc.libsodium.org/secret-key_cryptography/aead) / [XChaCha20-Poly1305](https://doc.libsodium.org/secret-key_cryptography/aead/chacha20-poly1305) | Future Co-Project key epochs and authenticated project records | Random project keys, per-device sealed key envelopes, nonce/AAD-bound ciphertext, and rotation after trust changes | Research/design reference only at this checkpoint; no project-content crypto code is shipped yet | ISC; compatible in principle, but the exact dependency and protected keyring still require review | The current server still stores shared-content plaintext. Before selecting this design, bind every envelope to project/epoch/recipient, require owner signatures, reject downgrade/replay, and keep project-wrap keys separate from private-message keys. |
