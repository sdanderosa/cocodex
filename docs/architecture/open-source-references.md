...
| [libsodium AEAD](https://doc.libsodium.org/secret-key_cryptography/aead) / [XChaCha20-Poly1305](https://doc.libsodium.org/secret-key_cryptography/aead/chacha20-poly1305) | Project key epochs and authenticated project records | Random project keys, per-device sealed key envelopes, nonce/AAD-bound ciphertext, and rotation after trust changes | Reuse `libsodium-wrappers-sumo` 0.8.2 for the shipped project-wrap and encrypted-context slice; do not copy source | ISC; compatible and covered by the existing notice review | The server stores opaque key/context envelopes and validates signatures/revisions without decrypting. The slice covers Final Goal/context only; chat, prompts, tasks, results, artifacts, and key rotation remain explicitly incomplete. |
