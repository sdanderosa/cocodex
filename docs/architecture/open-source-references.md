...
| [Windows DPAPI](https://learn.microsoft.com/windows/win32/api/dpapi/nf-dpapi-cryptprotectdata) / [.NET ProtectedData](https://learn.microsoft.com/dotnet/api/system.security.cryptography.protecteddata) | Windows user-bound local custody for exportable private keys | `CurrentUser` normally binds protected bytes to the same Windows logon and machine; purpose entropy separates protocol roles | Use the installed Windows API through the `SystemRoot`-derived absolute PowerShell/.NET surface; no copied source, binary, or added package | Windows platform API, not an open-source dependency; no redistributed implementation or new dependency | Private-alpha Client and Server private keys are purpose-bound DPAPI ciphertext plus current-user NTFS ACLs. The API does not stop malware already running as that user or protect unlocked process memory. Legacy PEM migrates atomically; Server recovery rewraps to the restoring user. TPM/CNG non-exportable adapters remain preferred where the signing, X25519, TLS, and recovery runtimes can support them coherently. See ADR 0047. |
...
- **Device identity:** independently adapt Syncthing's cryptographic
  ...
  NTFS ACLs. Raw legacy PEM is atomically migrated, and complete Server restore
