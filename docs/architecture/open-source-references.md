...
| [restic](https://github.com/restic/restic) / [design](https://github.com/restic/restic/blob/master/doc/design.rst) | Encrypted backup repositories, password-derived keys, authenticated restore | Encrypt all backup content, use a memory-hard password KDF, authenticate before accepting plaintext, and verify snapshots during restore | Concepts only; CoCodex copies no source or format and adds no dependency | BSD-2-Clause; compatible, with no notice required because no code is distributed | CoCodex uses a single bounded structured recovery archive with scrypt and AES-256-GCM from Node crypto. It contains only six fixed logical entries, validates identity/TLS/database relationships, and atomically swaps a stopped state root with rollback. It is same-identity disaster recovery, not authority migration. See ADR 0046. |
...
| [Tauri](https://github.com/tauri-apps/tauri) / [sidecars](https://v2.tauri.app/develop/sidecar/) | Native desktop delivery and a pinned local Client runtime | Keep the webview as a thin UI shell; bundle target-triple external binaries and supervise them from native code while leaving collaboration, device, and agent authority in the existing processes | Reuse the maintained Tauri 2 CLI, Rust runtime, and `tauri-plugin-shell` 2.3.5; no Tauri source is copied | MIT or Apache-2.0 where applicable; compatible with CoCodex MIT, with Cargo/npm lockfiles and notices preserved | Rust may launch only the configured `cocodex-runtime` sidecar; the webview retains only `core:default` and receives no shell/filesystem permission. The Client reuses a strictly identity-checked loopback proxy or supervises its owned child, but never embeds the separate CoCodex Server. Test CSP, exact Tauri-origin CORS/capabilities, child ownership, restart, installer contents, and packaged startup. See ADR 0049. |
...
| [Windows DPAPI](https://learn.microsoft.com/windows/win32/api/dpapi/nf-dpapi-cryptprotectdata) / [@primno/dpapi](https://github.com/primno/dpapi) | Windows user-bound local custody for exportable private keys | `CurrentUser` normally binds protected bytes to the same Windows logon and machine; purpose entropy separates protocol roles | Reuse exact `@primno/dpapi` 2.0.1 N-API bindings and `node-gyp-build` 4.8.4 instead of a custom native wrapper or PowerShell helper | Both dependencies are MIT; compatible, exact versions are locked, and notices are preserved | Private-alpha Client and Server private keys are purpose-bound DPAPI ciphertext plus current-user NTFS ACLs. The native binding is embedded in compiled Windows executables and fails closed if unavailable. The API does not stop malware already running as that user or protect unlocked process memory. Legacy PEM migrates atomically; Server recovery rewraps to the restoring user. TPM/CNG non-exportable adapters remain preferred where the signing, X25519, TLS, and recovery runtimes can support them coherently. See ADR 0047. |
...
- **Disaster recovery:** independently adapt restic's encrypt-everything,
  password-KDF, authenticate-before-restore principles for one fixed-entry
  Server archive. Preserve the old root as rollback after an atomic stopped
  restore; never confuse same-identity recovery with a planned authority
  handoff. No restic source, format, or dependency is used. See ADR 0046.
...
- **Device identity:** independently adapt Syncthing's cryptographic
  ...
  the operating system. On Windows, the Client's signing, messaging, and
  project-wrap private keys and the Server's authority and TLS private keys
  are stored as purpose-bound DPAPI `CurrentUser` ciphertext under current-user
  NTFS ACLs through the exact MIT-licensed `@primno/dpapi` 2.0.1 N-API
  binding. Raw legacy PEM is atomically migrated, and complete Server restore
  rewraps keys to the restoring user. This is an at-rest boundary, not a claim
  against same-user malware or unlocked process-memory compromise; non-Windows
  compatibility retains only the explicitly labeled filesystem boundary.
  See ADR 0047. Exactly one first device has a permanent local-only
