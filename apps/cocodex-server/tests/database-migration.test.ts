...
describe("CoCodex database migrations", () => {
  test("preserves legacy unsigned agent tables while installing signed task schema", async () => {
    ...
    try {
      ...
      expect(migrated.query(
        "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('project_key_envelopes', 'encrypted_project_context') ORDER BY name",
      ).all()).toEqual([{ name: "encrypted_project_context" }, { name: "project_key_envelopes" }]);
